...
main(int argc, char **argv)
{
    ...
    int Counts=0;                     // Counts is to count how many different -x files are input.
    char *UserProgram[100]={""};      //UserProgram is an array to store the different program name with multiple -x flags.  
#ifndef FILESYS_STUB
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    UserProgram[Counts] = argv[i + 1];//use UserProgram to store user programs names with multiple -x flags
	    i++;
...
Thread *t=new Thread(UserProgram[i]);// fork a new thread
t->Fork((VoidFunctionPtr) RunUserProg,(void *) UserProgram[i]);//Use RUnUserProg and give the function required argument UserProgram[i], which is the stored programs names to run.
}
/*
...
*/
   for(int i=0;i<Counts;i++){
   printf("Program [%d] =  %s\n",i, UserProgram[i]);//use for loop to output all the stored programs names in the array.
}
