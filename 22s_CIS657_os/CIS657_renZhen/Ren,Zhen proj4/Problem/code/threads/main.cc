...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    bool runUserProg = false; // jcoh
    char *userProgNames[10];  // joch up to 10 programs
    bool threadTestFlag = false;
    bool consoleTestFlag = false;
    bool networkTestFlag = false;
    int programCount = 0; //count of number of programs
#ifndef FILESYS_STUB
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    runUserProg = true; // jcoh
	    userProgNames[programCount] = argv[i + 1]; // jcoh
	    programCount++; // jcoh
	    i++;
	}
    ...
    if (runUserProg) { // jcoh
       for(int i = 0; i < programCount; i++){
            Thread * t = new Thread(userProgNames[i]);
            t->Fork((VoidFunctionPtr)RunUserProg, userProgNames[i]);
        }
    }
