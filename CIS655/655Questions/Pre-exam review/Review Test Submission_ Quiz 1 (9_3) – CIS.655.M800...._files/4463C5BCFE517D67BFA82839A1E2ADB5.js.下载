...
var Draggables = {
  ...
  endDrag: function(event) {
    ...
    this.activeDraggable = null;
    Sortable.restoreIFrames(this.element);
  },
...
Draggable.prototype = {
  ...
  startScrolling: function(speed) {
    ...
    this.scrollSpeed = [(speed[0]*this.options.scrollSpeed)/2,(speed[1]*this.options.scrollSpeed)/2];
  ...
  scroll: function() {
    ...
    if(this.options.scroll == window) {
      ...
      this.options.scroll.scrollLeft += this.scrollSpeed[0] * delta / 2000;
      this.options.scroll.scrollTop  += this.scrollSpeed[1] * delta / 2000;
    }
    ...
    if (this._isScrollChild) {
      ...
      Draggables._lastScrollPointer[0] += this.scrollSpeed[0] * delta / 2000;
      Draggables._lastScrollPointer[1] += this.scrollSpeed[1] * delta / 2000;
...
var Sortable = {
  ...
  restoreIFrames: function (element) {
    //Fix for problem when element is undefined.
    if ( !element )
    {
      return;
    }
    var iframes = Sortable.getAllIFrames(element);
    for (var ifr = 0; ifr < iframes.length; ifr++) {
      var frame = iframes[ifr];
      if (frame.bbsanitized) {
        frame.src = frame.tempSrc;
        frame.tempSrc = '';
        frame.bbsanitized = false;
      }
    }
  },

  getAllIFrames: function ( element )
  {
    return element.getElementsByTagName( "iframe" );
  },

  sanitizeIFrame: function ( element )
  {
    //Fix for problem when element is undefined.
    if ( !element )
    {
    ...
    const iframe = Sortable.getAllIFrames( element );
    for ( var ifr = 0; ifr < iframe.length; ifr++ )
    {
      var frame = iframe[ ifr ];
      if ( !frame.bbsanitized )
      {
        frame.bbsanitized = true;
        frame.tempSrc = frame.src;
        frame.src = '';
      }
    }
    return iframe;
  ...
  onEmptyHover: function(element, dropon, overlap) {
    ...
    if(!Element.isParent(dropon, element)) {
      ...
      Sortable.sanitizeIFrame(element);
