...
public:
	...
	int f(int a, int b) const { return pow(a, b); }


};

void Task_MemberFunction() {//I tried different cases, but only bind works
	myClass m;
	packaged_task<int()> task(bind(&myClass::f, &m, 2, 17));
	future<int> Fut = task.get_future();
	task();
	cout << "MemberFunction " << Fut.get() << endl;
