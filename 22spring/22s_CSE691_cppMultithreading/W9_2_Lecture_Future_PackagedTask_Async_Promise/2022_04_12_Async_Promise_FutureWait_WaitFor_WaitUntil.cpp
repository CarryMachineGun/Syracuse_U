...
int main() {
	future<bool> Fut1{ async(Is_Prime, 3478123) };
	...
	future<bool> Fut8 = async(Is_Prime, 23456712);

	if (Fut8.wait_for(chrono::nanoseconds(10)) == future_status::timeout) {

		cout << "Fut8 " << Fut8.get() << "   Timeout" << endl;
	...
	future_status::timeout      if the async is completed withiut  time interval 
