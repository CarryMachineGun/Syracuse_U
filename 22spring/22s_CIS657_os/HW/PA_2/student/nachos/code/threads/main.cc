...
void RunUserProg(void *filename)
{
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-x") == 0)
        {
            ...
            user_prog_name_list.Append(argv[i + 1]);
            i++;
        }
    ...
    // run user programs
    int prog_count = 0;
    while(!user_prog_name_list.IsEmpty())
    {
        // RunUserProg(user_prog_name_list.Front());
        printf("Program [%d] = %s \n", prog_count++ , user_prog_name_list.Front());
        user_prog_name_list.RemoveFront();
    }
