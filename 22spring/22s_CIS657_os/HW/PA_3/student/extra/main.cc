...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    List<char*> userProgNames; // List to hold multiple user programs
    ...
    // Iterate though the userProgNames and spawn a thread for
    // each program
    //int prog_count = 0;
    //
    while(!userProgNames.IsEmpty()){
      char *progName = userProgNames.RemoveFront();
      ASSERT (progName != NULL); // progName can't be NULL or else ABORT.

      // print prognames (For PA-2)
      // cout << "Program [" << prog_count++ << "] = " << progName << endl;
      
      /** <YOUR CODE GOES HERE>
       * Think about spawning a user-level thread 
       * and run each user program. It's name is stored progName variable.
       */
      Thread *T1 = new Thread(progName);
      T1->Fork(RunUserProg, progName);
